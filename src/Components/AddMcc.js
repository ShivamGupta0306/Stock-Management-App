...
const AddPlant = () => {
  ...
  useEffect(()=>{
    async function fetchCompany(){
        try{
            ...
            setPlant(response.data.result.Table);
        }
        catch(err){
            console.error(err);
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('http://182.18.144.204:50019/mcc/add', formDataWithCompany);
  ...
  const colourOptions = Plant.map((plant) => ({
  ...
  return (
    <div className="container mx-auto pt-10 mt-20 h-screen lg:h-full mb-5">
      <h2 className="text-3xl font-semibold mb-4">Add MCC</h2>
      ...
      <div className="mb-4">
      <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="Plant_Id">
            Plant Name
    </label>
      <Select
        ...
        value={colourOptions.find(option => option.value === Plant_Id)}
        onChange={(selectedOption) => setPlant_Id(selectedOption.value)}
      />
      </div>
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="MCC_Name">
            MCC Name
          </label>
          <input
            type="text"
            id="MCC_Name"
            name="MCC_Name"
            value={formData.MCC_Name}
            onChange={handleChange}
            placeholder="Enter MCC Name"
