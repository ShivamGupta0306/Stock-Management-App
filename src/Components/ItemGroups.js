...
const UserList = () => {
  ...
  const filteredUsers = users?.filter((user) => {
    ...
    const ItemCodeString = user.ItemMasterGroupCode.toString();
    const getSafeValue = (value) => (value ? value.toLowerCase() : 'Not Available');

    return (
      getSafeValue(user.ItemGroupName).includes(lowercaseSearchTerm) ||
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'ItemGroupList');
    XLSX.writeFile(wb, 'ItemGroupList.xlsx');
  };
    ...
    <div className='pt-10 mt-20'>
      <div className="container mx-auto pb-10">
        ...
        <div className="flex justify-end items-center gap-x-3 mb-2">
              <Link to="/createitemgroup" className="bg-blue-500 rounded-md px-2 py-1 hover:bg-blue-700 text-lg font-semibold">
            ...
            <table className="min-w-full min-h-full bg-white border border-gray-300 mb-10">
              <thead className="bg-blue-900">
                <tr className="text-white">
                  <th className="py-2 px-4 border-b">Item Group Code</th>
              ...
              <tbody>
                {currentUsers?.map((user) => (
                  <tr key={user.ItemMasterGroupCode}>
                    <td className="text-center py-2 px-4 border-b">{user.ItemMasterGroupCode}</td>
                    ...
                    <td className="text-center py-2 px-4 border-b">
                      <Link to={`/edit-group/${user.ItemMasterGroupCode}`}>
