...
const UserList = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await api.post('http://182.18.144.204:50019/mcc/get', {});
        setTimeout(() => {
          setLoading(false);
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const userIdString = user.MCC_Id.toString();
    ...
    return (
      getSafeValue(user.MCC_Name).includes(lowercaseSearchTerm) ||
  ...
  return (
    <div className='container h-screen lg:h-full mx-auto mt-20 pt-10'>
      <div className='mb-4 flex justify-between items-center'>
        ...
        <div className="flex gap-x-3">
          <Link to="/createmcc" className="bg-blue-500 rounded-md px-2 py-1 hover:bg-blue-700 text-lg font-semibold">
            ...
            <table className="min-w-full bg-white border border-gray-300">
              ...
              <tbody>
                {currentUsers.map((user) => (
                  <tr key={user.MCC_Id}>
                    <td className="text-center py-2 px-4 border-b">{user.MCC_Name || 'Not Available'}</td>
                    <td className="text-center py-2 px-4 border-b">{user.MCC_Code || 'Not Available'}</td>
                    ...
                    <td className="text-center py-2 px-4 border-b">{user.Is_Active ? 'Active' : 'Inactive'}</td>
                    <td className="text-center py-2 px-4 border-b">
                      <Link to={`/edit-mcc/${user.MCC_Id}`}>
