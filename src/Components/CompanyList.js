...
const UserList = () => {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const userIdString = user.CId.toString();
    const phonenoString = user.MobileNo ? user.MobileNo.toString() : '3245245';
    const getSafeValue = (value) => (value ? value.toLowerCase() : 'Not Available');

    return (
      getSafeValue(user.CompanyName).includes(lowercaseSearchTerm) ||
        ...
        <div className="container mx-auto pb-10">
          <div className='flex justify-between items-center mb-4'>
            ...
            <div className="flex justify-end items-center gap-x-3 mb-2">
              <Link to="/create-company" className="bg-blue-500 rounded-md px-2 py-1 hover:bg-blue-700 text-lg font-semibold">
                ...
                <table className="min-w-full bg-white border border-gray-300">
                  ...
                  <tbody>
                    {currentUsers.map((user) => (
                      <tr key={user.UserID}>
                        <td className="text-center py-2 px-4 border-b">{user.CompanyName}</td>
                        <td className="text-center py-2 px-4 border-b">{user.CId || 'Not Available'}</td>
                        <td className="text-center py-2 px-4 border-b">{user.MobileNo}</td>
                        <td className="text-center py-2 px-4 border-b">{user.CompanyAddress}</td>
                        <td className="text-center py-2 px-4 border-b">
                          {user.Is_Active ? <span className="text-green-500">Active</span> : <span className="text-red-500">Inactive</span>}
                        </td>
                        <td className="text-center py-2 px-4 border-b">
                          <Link to={`/edit-company/${user.CompanyCode}`}>
